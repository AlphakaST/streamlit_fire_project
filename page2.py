...
with st.form("form"):
    ...
    if submit_button and mydist1 and mydist2 and student_name:
        ...
        # Google Sheets로 업데이트
        new= pd.DataFrame([{"name":student_name, "startmonth":mydist1, "basis":mydist2}])
        update = pd.concat([data, new], ignore_index=True)
        conn.update(worksheet="page2_1", data = update)
...
with st.form("form2"):
    ...
    if submit_button and mydist3 and mydist4 and student_name:
        ...
        # Google Sheets로 업데이트
        new2= pd.DataFrame([{"name":student_name, "parameter":mydist3, "basis":mydist4}])
        update2 = pd.concat([data2, new2], ignore_index=True)
        conn.update(worksheet="page2_2", data = update2)
